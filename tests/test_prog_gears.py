from redgrease import GearsBuilder as GRB
from redgrease import execute, hashtag
...
def test_aggregateby(env):
    ...
    res = (
        getGB(env)
        .map(lambda x: x["value"])
        .aggregateby(lambda x: x, 0, lambda k, a, r: a + int(r), lambda k, a, r: a + r)
        .map(lambda x: (x["key"], x["value"]))
        .sort()
        .run()
    )
...
def test_register(env):
    ...
    env.expect("RG.TRIGGER", "test", "this", "is", "a", "test").equal(
