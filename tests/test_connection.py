...
def test_basic_redis_functions(redis_instance: Redis):
    ...
    assert redis_instance.set(key, value)
    # Ensure that the newly set key is set
    assert redis_instance.keys(key) == [key.encode()]
    # Ensure that the newly set key has the right valued
    assert redis_instance.get(key) == value.encode()
    # Ensure that there are no set keys after we delete the newly added ke
    assert redis_instance.delete(key)
    # Ensure that the key is not defined
    assert not redis_instance.keys(key)


def test_basic_gears_functions(redis_instance: Redis):
    """Test that the Redis test server has the Gears Module loaded"""
    assert redis_instance.execute_command("RG.PYEXECUTE GB().run()")
